...
const Maps=()=>{
    ...
    useEffect(()=>{
        ...
        fetch(`https://sebastian.armata:Dragon1122@opensky-network.org/api/states/all?lamin=${bounds.lamin}&lomin=${bounds.lomin}&lamax=${bounds.lamax}&lomax=${bounds.lomax}`)
    ...
    return(
        <MapContainer center={position} zoom={9} scrollWheelZoom={true} style={{height: "100vh"}}>
            ...
            />
            {markers && markers.map(marker=><Marker id={marker[0]} position={[marker[6],marker[5]]} icon={<img src={plane} alt="p" style={{width:'20px'}}></img>}>
                <Popup>
